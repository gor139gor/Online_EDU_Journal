...
class Program
{
    ...
    static void PopulateScheduleTable(NpgsqlConnection connection)
    {
        ...
        for (int classGroupId = 1; classGroupId <= 22; classGroupId++)
        {
            foreach (string dayOfWeek in daysOfWeek)
            {
                ...
                // Визначення вчителя, пов’язаного з цим предметом, у TeacherSubjects
    ...
    static void PopulateStudentPerformanceTable(NpgsqlConnection connection)
    {
        ...
        // Заповнення StudentPerformance для кожного розкладу, його класу та відповідних студентів відповідними предметами
        foreach (int scheduleId in scheduleIds)
        {
            int classGroupId = -1; // утримання класної групи за поточним розкладом
            ...
            // Заповніть StudentPerformance для поточного розкладу, учнів класної групи та її предметів
            foreach (int studentId in studentIds)
            {
                foreach (int subjectId in subjectIds)
                {
                    DateTime date = DateTime.Now.AddDays(random.Next(-30, 0)); // Випадкова дата в межах останнього місяця
                    string status = (random.Next(2) == 0) ? " " : " н "; // Випадковий статус
                    int grade = (status == " ") ? random.Next(1, 13) : 0; // Випадкова оцінка
